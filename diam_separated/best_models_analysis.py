...
def save_importance_bar_xgb_only(importance_df, filename):
    fig, ax = plt.subplots(figsize=(10, 6))
    df_sorted = importance_df.sort_values('XGB_Importance', ascending=False)
    variables = [VARIABLE_LABELS.get(v, v) for v in df_sorted['Variable']]
    y_pos = np.arange(len(variables))
    ax.barh(y_pos, df_sorted['XGB_Importance'].values, color=magma_cmap(0.7))
    ax.set_yticks(y_pos)
    ax.set_yticklabels(variables)
    ax.set_xlabel('Importance Score', fontsize=20, fontfamily='Arial')
    ax.set_xlim(0, 1.0)
...
save_importance_bar_xgb_only(a1_importance_df, 'A1_WEEP_Importance_XGB_only.png')
...
save_importance_bar_xgb_only(a2_importance_df, 'A2_FLOOD_Importance_XGB_only.png')
...
save_importance_bar_xgb_only(a3_importance_df, 'A3_CONV_Importance_XGB_only.png')
...
save_importance_bar_xgb_only(a4_importance_df, 'A4_PURITY_Importance_XGB_only.png')
